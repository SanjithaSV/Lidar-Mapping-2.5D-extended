...
def predict(pts4, model, cfg, batch=256, seed=0, with_prov=False, with_clusters=False):
    """
    ...
    if with_clusters:
        # cluster ids are aligned to the full scan; -1 means ground/noise.
        full_cl = np.full(N, -1, np.int64)
        full_cl[fg] = cl
        if with_prov:
            return lab, out, prov, full_cl, pred
        return lab, out, full_cl, pred
