...
const pal = () => { if (!SP){ SP = shades(SPECTRAL); CL = shades(CLASSC);
  TV = shades([css('--ink3'), css('--no'), css('--ok')]); DT = shades(DETC); MT = shades([css('--ink3'), css('--ok'), css('--bad')]); } };
...
function tiersOf(f){
  ...
  if (!d._t) d._t = d.tiers.map(t => ({res:t.res, x0:t.x0, y0:t.y0, nx:t.nx, ny:t.ny,
    ...
    det:t.det ? dec(t.det,Uint8Array) : null, motion:t.motion ? dec(t.motion,Uint8Array) : null}));
  return d._t;
}
...
function draw(){
  ...
  const P = S.paint==='height'?SP : S.paint==='cls'?CL : S.paint==='det'?DT : S.paint==='motion'?MT : TV;
  let cur=-1;
  ...
  for (let o=0;o<qn;o++){
    ...
    else if (S.paint==='motion'){ const v = Tt.motion ? Tt.motion[p0] : 255; ci = v===255 ? 0 : v+1; }
...
function legend(){
  ...
  if (S.paint === 'height'){
    ...
    box.innerHTML = CLASSN.map((n,i)=>
      ...
      + `<div class="ends" style="margin-top:5px"><span>${d.source==='model'?'only car and pedestrian come from the network':'SemanticKITTI semantic + instance labels'}</span></div>`;
  } else if (S.paint === 'det'){
    if (d.source !== 'model'){
      box.innerHTML = `<div class="ends"><span>detector provenance is unavailable in ground-truth mode</span></div>`;
      return;
    }
  ...
  } else if (S.paint === 'motion'){
    const mm = d.motion || {};
    box.innerHTML = [['--ok','static'],['--bad','dynamic'],['--ink3','unknown']]
      ...
      + `<div class="ends" style="margin-top:5px"><span>${fmt(mm.dynamic_objects||0)} dynamic · ${fmt(mm.static_objects||0)} static object clusters${d.source==='truth' ? ` · GT ${fmt(mm.gt_dynamic_objects||0)} dynamic / ${fmt(mm.gt_static_objects||0)} static` : ''}</span></div>`;
  } else {
